...
### 3.2 Схематичное представление задачи
...
## 4. Построение целевых переменных (таргетов)
...
### 4.2 Построение таргетов для разных горизонтов
...
### 5.1 Подход 1: Отдельная модель для каждого горизонта (12 моделей)
...
### 5.2 Подход 2: Одна модель с признаком горизонта
...
### 5.3 Подход 3: Моделирование интенсивности дефолта (Forward Intensity)
...
### 6.1 Основная метрика: Gini = 2×ROC-AUC - 1
...
## 7. Требования к результатам модели

### 7.1 Кривая вероятности дефолта на 12 точек
...
### 7.3 Монотонность кривой (неубывающая)
...
### 7.4 Поведение кривой перед дефолтом
...
## 8. Технические требования к модели

### 8.1 Сохранение модели в формате pickle
...
### 8.3 Интерпретируемость (для логистической регрессии/оптимизации)
